...
// MOD begin of sub product
    ...
    $products_name =
                       ...
                       '    <td class="text-left hide-on-mobile-portrait"><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $products_parent_id['products_parent_id']) . '"><b>' . $products[$i]['name'] . '</b></a>';
    } else {
    $products_name =
                       '  ' .
...
<div class="clearfix">
   <?php
      ...
      if (trim(strip_tags($returned_rci)) != NULL) {
        ...
        echo '  <div class="clearfix"><span class="pull-right ot-total">' . SUB_TITLE_TOTAL . ' &nbsp; ' . $currencies->format($cart->show_total()) . '</span></div>' . "\n";
      }
