...
echo $cre_RCI->get('affiliatebannersbuild', 'top');
// RCI code eof
?>
...
<?php
 ...
 if (tep_not_null($affiliate_pbanners)){
   if(!isset($_SESSION['affiliate_id'])) {
     ...
     $affiliate_id = (int)$_SESSION['affiliate_id'];
      ...
      switch (AFFILIATE_KIND_OF_BANNERS) {
        case 1:
        $link_img = '<a href="' . HTTP_SERVER . DIR_WS_CATALOG . FILENAME_PRODUCT_INFO . '?ref=' . $affiliate_id . '&products_id=' . $individual_banner_id . '" target="_blank"><img src="' . (($request_type == 'SSL') ? HTTPS_SERVER : HTTP_SERVER) . DIR_WS_CATALOG . DIR_WS_IMAGES . $affiliate_pbanners['products_image'] . '" border="0" alt="' . $affiliate_pbanners['products_name'] . '"></a>';
        $link_txt = '<a href="' . HTTP_SERVER . DIR_WS_CATALOG . FILENAME_PRODUCT_INFO . '?ref=' . $affiliate_id . '&products_id=' . $individual_banner_id . '" target="_blank">' . $affiliate_pbanners['products_name'] . '</a>';
      break;
      case 2:
   // Link to Products
        $link_img = '<a href="' . HTTP_SERVER . DIR_WS_CATALOG . FILENAME_PRODUCT_INFO . '?ref=' . $affiliate_id . '&products_id=' . $individual_banner_id . '" target="_blank"><img src="' . (($request_type == 'SSL') ? HTTPS_SERVER : HTTP_SERVER) . DIR_WS_CATALOG . FILENAME_AFFILIATE_SHOW_BANNER . '?ref=' . $affiliate_id . '&affiliate_pbanner_id=' . $individual_banner_id . '" border="0" alt="' . $affiliate_pbanners['products_name'] . '"></a>';
        $link_txt = '<a href="' . HTTP_SERVER . DIR_WS_CATALOG . FILENAME_PRODUCT_INFO . '?ref=' . $affiliate_id . '&products_id=' . $individual_banner_id . '" target="_blank">' . $affiliate_pbanners['products_name'] . '</a>';
      break;
     }
echo '     <tr>' . "\n";
echo '      <td valign="top">' . "\n";
            ...
            $info_box_contents[] = array('text' => TEXT_AFFILIATE_NAME . '&nbsp; ' . $affiliate_pbanners['products_name']);
...
// RCI code start
echo $cre_RCI->get('affiliatebannersbuild', 'menu');
// RCI code eof
// BOF: Lango Added for template MOD
if (MAIN_TABLE_BORDER == 'yes'){
...
<?php
// RCI code start
echo $cre_RCI->get('affiliatebannersbuild', 'bottom');
