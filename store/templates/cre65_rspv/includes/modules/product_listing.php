      ...
      for ($col=0, $n=sizeof($column_list); $col<$n; $col++) {
        ...
        switch ($column_list[$col]) {
          ...
          case 'PRODUCT_LIST_NAME':
            ...
            $lc_name = '<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, ($cPath ? 'cPath=' . $cPath . '&amp;' : '') . 'products_id=' . $listing[$x]['products_id'] . $params) . '">' . mb_strimwidth(str_replace(array('- Add-On',' - Module'),'',$listing[$x]['products_name']),0,24) . '</a>';
            $blurb_query = tep_db_query("select products_blurb from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . (int)$listing[$x]['products_id'] . "' and language_id = '" . (int)$languages_id . "'");
            $blurb_result = tep_db_fetch_array($blurb_query);
            if(tep_not_null($blurb_result['products_blurb'])) {
                $lc_blurb = truncate($blurb_result['products_blurb']);
            }
            break;
          ...
          case 'PRODUCT_LIST_PRICE':
            ...
            $lc_price = $pf->getPriceStringShort();
            break;
          ...
          case 'PRODUCT_LIST_IMAGE':
            ...
            $lc_img = '<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, ($cPath ? 'cPath=' . $cPath . '&amp;' : '') . 'products_id=' . $listing[$x]['products_id'] . $params) . '">' . tep_image(DIR_WS_IMAGES . $listing[$x]['products_image'], $listing[$x]['products_name'], 400, 250, 'class="group list-group-image"') . '</a>';
            break;
          ...
          case 'PRODUCT_LIST_BUY_NOW':
              ...
              if ($valid_to_checkout == true) {
                ...
                if ($hide_add_to_cart == 'false' && group_hide_show_prices() == 'true') {
                  ...
                  $lc_button = '<a class="btn btn-success" href="' . tep_href_link(basename($PHP_SELF), tep_get_all_get_params(array('action','cPath','products_id')) . 'action=buy_now&amp;products_id=' . $listing[$x]['products_id'] . '&amp;cPath=' . tep_get_product_path($listing[$x]['products_id']) . $params) . '">Add to cart</a>';
                }
              }
              $lc_text .= '</div>';
              break;
      ...
      //$lc_text = implode( $product_contents);  
        ...
        <div class="item  col-xs-12 col-sm-6 col-md-3 col-lg-4">
            <div class="thumbnail equalize">
                <?php echo $lc_img;?>
                <div class="caption">
                    <h4 class="group inner list-group-item-heading"><?php echo $lc_name;?></h4>
                    <p class="group inner list-group-item-text"><?php echo cre_clean_html($lc_blurb);?></p>
                    <div class="row" style="margin-top:10px;">
                        <div class="col-xs-12 col-md-6">
                            <p class="lead"><?php echo $lc_price;?></p>
                        </div>
                        <div class="col-xs-12 col-md-6">
                            <?php echo $lc_button;?>
                        </div>
